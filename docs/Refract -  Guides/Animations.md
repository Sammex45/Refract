    ...
    <div>
      ...
      <div
        animate={{
          ...
          borderRadius: isAnimating.value ? ['10%', '50%', '10%', '50%', '10%'] : '10%',
          transition: {
            duration: 2,
...
const SwipeCards = createComponent(({ lens }) => {
  ...
  const handleDragEnd = () => {
    ...
    if (Math.abs(dragX.value) > 100) {
      ...
      } else if (dragX.value < 0 && currentIndex.value < cards.value.length - 1) {
        currentIndex.set(currentIndex.value + 1)
      }
    }
  ...
  return (
    <div>
      <div style={{ marginBottom: '20px', display: 'flex', gap: '10px' }}>
        ...
        <button onClick={() => layout.set(layout.value === 'grid' ? 'list' : 'grid')}>
      ...
      <div
        style={{
          ...
          gridTemplateColumns: layout.value === 'grid' ? 'repeat(3, 1fr)' : 'none',
          ...
          <div
            ...
            animate={{
              ...
              transform: animating.value 
                ? `translateY(${Math.sin(Date.now() / 1000 + i) * 10}px) scale(${1 + Math.sin(Date.now() / 1000 + i) * 0.1})`
  ...
  return (
    <div>
        ...
        {Object.keys(pages).map(page => (
          <button
            ...
            style={{
              padding: '10px 20px',
              margin: '0 5px',
              background: currentPage.value === page ? pages[page].color : '#e2e8f0',
      ...
      >
        ...
        <p>This is the {pages[currentPage.value].title.toLowerCase()} page content.</p>
      </div>
    </div>
  )
})
