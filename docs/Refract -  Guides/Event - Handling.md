...
const FormExample = createComponent(({ lens }) => {
  ...
  return (
    <div>
      ...
      <img src="/logo.png" onDragStart={handleDragStart} alt="Not draggable" />
    </div>
  );
});
```
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: "20px" }}>
        <p>
          Last key: <strong>{lastKey.value}</strong>
        </p>
...
const MouseTracker = createComponent(({ lens }) => {
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "white", padding: "20px" }}>Move your mouse around</p>
    </div>
  );
});
```
...
const DragDropList = createComponent(({ lens }) => {
  ...
  const handleDrop = (e, targetItem) => {
    ...
    const draggedIdx = items.value.findIndex(
...
const SomeComponent = createComponent(() => {
  ...
  return (
    <div>
      ...
      <button onClick={() => notify("Error!", "error")}>Show Error</button>
    </div>
  );
});
```
...
const Publisher = createComponent(({ lens }) => {
  ...
  return <button onClick={handleClick}>Publish Count: {count.value}</button>;
});
...
const Subscriber = createComponent(({ lens }) => {
  ...
  return <div>Received Count: {receivedCount.value}</div>;
});
```
...
<div onClick={handleClick}>
  {items.map((item) => (
    <button data-id={item.id}>{item.text}</button>
  ))}
</div>;
```
    ...
    const handleKeyDown = (e) => {
      switch (e.key) {
        ...
        case "ArrowDown":
          e.preventDefault();
          selectedIndex.set(
            Math.min(items.length - 1, selectedIndex.value + 1)
          );
          break;
        case "Enter":
...
_Events bring your application to life. With Refract's intuitive event system, you can create rich, interactive experiences with ease._
