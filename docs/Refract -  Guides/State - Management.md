...
const Profile = createComponent(({ lens }) => {
  ...
  return (
    <div>
      ...
      />
      <button onClick={incrementAge}>Birthday! (Age: {user.value.age})</button>
    </div>
  );
});
```
...
const Header = createComponent(({ lens }) => {
  ...
  return (
    <header className={`theme-${currentTheme.value}`}>
      {authenticated.value ? (
        <div>
          Welcome, {user.value?.name}!<button onClick={logout}>Logout</button>
        </div>
      ) : (
        <button onClick={() => isAuthenticated.set(true)}>Login</button>
      )}
...
const ShoppingCart = createComponent(({ lens }) => {
  ...
  const tax = lens.useComputed(() => subtotal.value * taxRate.value);

  const total = lens.useComputed(() => subtotal.value + tax.value);
  ...
  return (
    <div>
      ...
      <p>
        Tax ({(taxRate.value * 100).toFixed(0)}%): ${tax.value.toFixed(2)}
      </p>
...
const createUndoable = (initialValue) => {
  ...
  const canUndo = derivedRefraction([currentIndex], ([idx]) => idx > 0);
...
const SearchResults = createComponent(({ lens }) => {
  return (
    <div>
      ...
      <button
        onClick={() => currentPage.set(String(Number(currentPage.value) + 1))}
      >
        Next Page
      </button>
    </div>
...
_State management doesn't have to be complex. With Refract's refractions, it's simple, predictable, and powerful._
